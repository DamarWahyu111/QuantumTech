      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 border-y border-white/10">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              <div className="text-4xl md:text-5xl font-bold text-[#26a8a8]">
                {hasTestimonials ? `${testimonials.length}+` : "100%"}
              </div>
              <div className="text-white/60 mt-2">{hasTestimonials ? "Klien Puas" : "Komitmen Kualitas"}</div>
            </div>
            <div>
            ...
            <div>
              <div className="text-4xl md:text-5xl font-bold text-[#86bc25]">
                {hasTestimonials
                  ? (testimonials.reduce((acc, t) => acc + t.rating, 0) / testimonials.length).toFixed(1)
                  : "5.0"}
              </div>
              <div className="text-white/60 mt-2">Rating Rata-rata</div>
            </div>
          </div>
        </div>
      </section>
                ...
                <div className="relative bg-gradient-to-br from-[#26a8a8]/20 to-[#86bc25]/20 rounded-3xl p-8 md:p-12 border border-white/10">
                  ...
                  <div className="text-center">
                    ...
                    <div className="flex items-center justify-center gap-4">
                      <div className="w-16 h-16 rounded-full border-2 border-[#26a8a8] bg-gradient-to-br from-[#26a8a8] to-[#86bc25] flex items-center justify-center text-white text-xl font-bold">
                        {filteredTestimonials[0].name.charAt(0).toUpperCase()}
                      </div>
                      <div className="text-left">
                        <div className="text-white font-semibold">{filteredTestimonials[0].name}</div>
                        <div className="text-[#26a8a8] text-sm">{filteredTestimonials[0].role || "Client"}</div>
                  ...
                  {filteredTestimonials.slice(1).map((testimonial) => (
                    ...
                    >
                      <div className="flex items-start gap-4 mb-4">
                        <div className="w-14 h-14 rounded-full border-2 border-white/20 group-hover:border-[#26a8a8] transition-colors bg-gradient-to-br from-[#26a8a8]/50 to-[#86bc25]/50 flex items-center justify-center text-white font-bold">
                          {testimonial.name.charAt(0).toUpperCase()}
                        </div>
                        <div className="flex-1">
                          <div className="text-white font-semibold">{testimonial.name}</div>
                          <div className="text-[#26a8a8] text-sm">{testimonial.role || "Client"}</div>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-[#26a8a8]/20 to-[#86bc25]/20">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-light text-white mb-6">
            {hasTestimonials ? (
              <>
                Sudah Menjadi Klien <span className="text-[#86bc25]">Kami?</span>
              </>
            ) : (
              <>
          ...
          <p className="text-white/60 text-lg mb-8">
            {hasTestimonials
              ? "Bagikan pengalaman Anda bekerja sama dengan QuantumTech. Testimoni Anda sangat berarti bagi kami!"
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link
              href="/#services"
              className={cn(
                "inline-flex items-center justify-center px-8 py-4 font-semibold rounded-full transition-colors",
                hasTestimonials
                  ? "bg-white/5 text-white border border-white/20 hover:bg-white/10"
                  : "bg-gradient-to-r from-[#26a8a8] to-[#86bc25] text-white hover:opacity-90",
              )}
            >
              Lihat Layanan Kami
            </Link>
